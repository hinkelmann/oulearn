...
module.exports = (env, argv) => { // eslint-disable-line no-undef
    ...
    if (fs.existsSync(path.resolve(projectRootPath, 'composer.lock'))) {
        ...
        const oulearnPackage = composerLock.packages.find((packageItem) => packageItem.name === 'oulearn/oulearn');
        oulearnVersion = oulearnPackage ? oulearnPackage.version : oulearnVersion;
    }
    ...
    return {
        ...
        plugins: [
            ...
            new webpack.DefinePlugin({
                OULEARN_BUILD_VERSION: JSON.stringify(oulearnVersion),
            }),
        ],
        resolve: {
            alias: {
            ...
            rules: [
                {
                    ...
                    exclude: /node_modules[/\\](?!(oulearn-(.*)-bundle|@ckeditor|ckeditor5|array-move|lodash-es|@react-leaflet|react-leaflet)[/\\])/,
                    use: {
                        loader: 'babel-loader',
                        options: {
                            cacheDirectory: true,
